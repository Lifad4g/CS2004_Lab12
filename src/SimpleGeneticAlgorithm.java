...
public class SimpleGeneticAlgorithm
{
	...
	private void DoUniformCrossOver(int p1id,int p2id)
	{
		...
		for(int i = 0; i < nbits; i++){
			...
			int x = CS2004.UI(0, 1);
			if(x == 0){
				//add p1 genes to c1 and p2 genes to c2
				c1.add(p1.get(i));
				//System.out.println("Added p1 gene to c1: " + i);
				c2.add(p2.get(i));
				//System.out.println("Added p2 gene to c2: " + i);
			}else if(x == 1){
				//add p2 genes to c1 and p1 genes to c2
				c1.add(p2.get(i));
				//System.out.println("Added p2 gene to c1: " + i);
				c2.add(p1.get(i));

				//System.out.println("Added p1 gene to c2: " + i);
			}
